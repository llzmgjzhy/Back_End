...
if ($result['type'] == 'text') {
    if (($result['obj']->Content) == "获取账户" || ($result['obj']->Content) == "获取账号") {
        ...
        if ($is_exist != null) {
            ...
            $sql = "SELECT * FROM user_info ORDER BY ASC";
            $backinfo = $conn->query($sql);
            $row = $backinfo->fetch_assoc();
            $showtext;
            if ($row != null) {
                $showtext = $row['user_token'];
                $sqlchange = "UPDATE user_info SET is_get='1' WHERE account='{$row['account']}'";
